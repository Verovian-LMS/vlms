...
const LectureDiscussion: React.FC<LectureDiscussionProps> = ({ lectureId }) => {
  ...
  // Fetch discussions and comments for this lecture
  useEffect(() => {
    const fetchDiscussions = async () => {
      try {
        setLoading(true);
        
        // First fetch discussions for this lecture
        const { data: discussionsData, error: discussionsError } = await supabase
          .from('discussions')
          .select('*, created_by:profiles!created_by(name, avatar)')
        ...
        if (discussionsData && discussionsData.length > 0) {
          ...
          // Then fetch comments for the first discussion
          const firstDiscussionId = discussionsData[0].id;
          await fetchCommentsForDiscussion(firstDiscussionId);
        } else {
          // If no discussions, create one for this lecture
          if (isAuthenticated) {
            const { data: newDiscussion, error: createError } = await supabase
              ...
              .insert({
                ...
                description: 'Share your thoughts and questions about this lecture',
                created_by: user?.id,
                is_pinned: false,
                is_closed: false
              })
    ...
    fetchDiscussions();
  }, [lectureId, user, isAuthenticated]);
  
  const fetchCommentsForDiscussion = async (discussionId: string) => {
    try {
      ...
      console.error('Error in fetchCommentsForDiscussion:', error);
    }
  };
    ...
    try {
      if (discussions.length === 0) {
        toast({
          ...
          description: "No discussion thread exists for this lecture yet.",
          variant: "destructive",
        });
        return;
      }
      ...
      // Refresh comments
      setNewComment('');
      await fetchCommentsForDiscussion(discussionId);
  ...
  const handleSubmitReply = async () => {
    ...
    try {
      ...
      setReplyContent('');
      setReplyingTo(null);
      await fetchCommentsForDiscussion(parentComment.discussion_id);
    ...
    <Card className="mt-6">
      ...
      <CardContent>
        {isAuthenticated ? (
          <div className="mb-8">
            <Textarea
              placeholder="Share your thoughts or questions about this lecture..."
...
// Add Link component at the top since it was missing in the original import
